...
public class HackmdCommand {
    ...
    int list(
        ...
        notes.sort(Comparator.comparing(Note::publishedAt, Comparator.nullsLast(Comparator.reverseOrder())));
    ...
    private static void printNotes(List<Note> notes) {
        ...
        System.out.format(format, "-".repeat(maxIdLength), "-".repeat(maxTitleLength), "-".repeat(19));

        for (Note note : notes) {
            String publishedAt = (note.publishedAt() == null) ? "N/A" : FORMATTER.format(note.publishedAt());
