...
class HackmdRestClientTest {
    ...
    void listsNotesWithAuthorization() {
        ...
        wireMock.verifyThat(
                getRequestedFor(urlEqualTo("/v1/notes")).withHeader("Authorization", equalTo("Bearer test-token")));
    }
    ...
    void createsNotesUsingTheCanonicalResponseType() {
        ...
        assertEquals(
                "note-1",
                restClient
                        .createNote(new CreateNoteRequest("Test note", "Body", "owner", "owner"))
                        .id());
